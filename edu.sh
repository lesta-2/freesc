...
wget -q -O /usr/local/bin/edu-ssh https://raw.githubusercontent.com/lesta-2/freesc/main/proxy-template.py
chmod +x /usr/local/bin/edu-ssh

# Installing Service
cat > /etc/systemd/system/edussh-nontls.service << END
[Unit]
Description=Ssh Websocket By Wisanggeni
...
wget -q -O /usr/local/bin/edu-ovpn https://raw.githubusercontent.com/lesta-2/freesc/main/edu-op.py
...
[Unit]
Description=Ovpn Websocket By Wisanggeni
Documentation=https://t.me/sampiiiiu
After=network.target nss-lookup.target
